  ...
  async renderViewer(main, sopId) {
    ...
    header.innerHTML = `
      ...
      <div class="tags" style="margin-top:8px;">
        <span>إعداد: <b>${esc(sop.prepared_by || "-")}</b> ${sop.prepared_at ? `(${esc(sop.prepared_at)})` : ""}</span>
        <span>مراجعة: <b>${esc(sop.reviewed_by || "-")}</b> ${sop.reviewed_at ? `(${esc(sop.reviewed_at)})` : ""}</span>
        <span>اعتماد: <b>${esc(sop.approved_by || "-")}</b> ${sop.approved_at ? `(${esc(sop.approved_at)})` : ""}</span>
      </div>
    `;
        ...
        stage.steps.forEach((step, stIdx) => {
          ...
          stepEl.innerHTML = `
            ...
            <div class="step-body">
              ...
              ` : ""}
              ${(step.accept_criteria || step.reject_criteria) ? `
                <div class="accept-reject">
                  ${step.accept_criteria ? `<div class="ar-ok">✔ قبول: ${esc(step.accept_criteria)}</div>` : ""}
                  ${step.reject_criteria ? `<div class="ar-bad">✘ رفض: ${esc(step.reject_criteria)}</div>` : ""}
                </div>
              ` : ""}
              ${step.defect_code ? `<div class="hint">كود العيب: <code>${esc(step.defect_code)}</code></div>` : ""}
              ${step.ppe_notes ? `<div class="hint">⚠️ ${esc(step.ppe_notes)}</div>` : ""}
